...
//
// Data architecture:
//   data/search-index.js  -> window.CENIVERSE_INDEX  (lightweight, loaded on
//                             every page, powers search/autocomplete only)
//   data/movies/<id>.js   -> window.CENIVERSE_MOVIE   (full record, loaded
//                             ONLY by movie.html, ONLY for the ?id= in the
//                             URL, via a dynamically-injected <script> tag)
//
// Script-tag loading (not fetch) is deliberate: fetch() of local files is
// blocked by the browser over file://, which is how most people will
// actually open these files. Plain <script src> works everywhere.
// ============================================================================
...
(function movieDetailLoader() {
  ...
  script.src = `data/movies/${requestedId}.js`;
  script.onload = () => {
    if (typeof window.CENIVERSE_MOVIE === 'undefined' || !window.CENIVERSE_MOVIE) {
      showNotFound();
      return;
    }
    document.body.dataset.movieSlug = requestedId;
    window.CENIVERSE_CURRENT_MOVIE = window.CENIVERSE_MOVIE;
    try {
      renderMovie(window.CENIVERSE_MOVIE);
      if (loadingEl) loadingEl.hidden = true;
  ...
  function renderMovie(m) {
    // ---- head / SEO ----
    document.title = `${m.title} (${m.year}) — Cast, Crew, Release Date & Real Data | CeniVerse`;
    ...
    if (poster) poster.dataset.tone = m.posterTone || 1;
    set('posterGlyph', m.glyph || m.title[0].toUpperCase());
    ...
    // Real financials instead of a fabricated rating — we're honest that we
    // haven't scored this title ourselves yet.
    let ratingRow = '';
    if (m.boxOffice) {
      ratingRow += `<div class="rating-badge"><div class="num" style="font-size:26px;">${m.boxOffice}</div><div class="lbl"><b>Worldwide Box Office</b>Verified figure</div></div>`;
    }
    if (m.budget) {
      ratingRow += `<div class="rating-badge"><div class="num" style="font-size:22px;color:var(--text-secondary);">${m.budget}</div><div class="lbl"><b>Budget</b>Reported figure</div></div>`;
    }
    if (!ratingRow) {
    ...
    (m.crew || []).forEach(c => crewRows.push([c.department || c.role, c.name]));
    setHtml('crewGrid', crewRows.map(([role, name]) => `
      <div class="crew-item"><div class="cr-role">${role}</div><div class="cr-name">${name}</div></div>`).join('')
    ...
    if (m.awards) prodRows.push(['Awards', m.awards]);
    setHtml('productionGrid', prodRows.map(([role, name]) => `
      <div class="crew-item"><div class="cr-role">${role}</div><div class="cr-name">${name}</div></div>`).join(''));
    ...
    if (sourceBlock) {
      sourceBlock.innerHTML = m.sourceNote ? `
        <div class="side-card" style="margin:0;">
          <h4>Data Source</h4>
          <p style="font-size:12.5px;color:var(--text-muted);line-height:1.7;">${m.sourceNote}. CeniVerse does not invent cast, crew, dates or box-office figures — anything we can't verify is simply left blank.</p>
        </div>` : '';
    }
    ...
    setHtml('alsoLikeList', related.map(r => `
      <a href="movie.html?id=${r.id}" style="display:flex;gap:10px;align-items:center;">
        <div style="width:44px;height:64px;border-radius:4px;overflow:hidden;flex-shrink:0;" class="ph-poster" data-tone="${r.posterTone || 1}"></div>
...
function initSiteSearch() {
  ...
  function search(query) {
    ...
    return index.filter(m => {
      ...
      if (m.language && m.language.toLowerCase().includes(q)) return true;
  ...
  function render(results, query) {
    ...
    suggestionsEl.innerHTML = results.map(m => `
      <a class="sg-item" href="movie.html?id=${m.id}">
        <div class="sg-thumb ph-poster" data-tone="${m.posterTone || 1}"></div>
        <div>
