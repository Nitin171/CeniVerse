...
(function movieDetailLoader() {
  ...
  function renderMovie(m) {
    ...
    if (poster) {
      ...
      if (m.poster) {
        poster.innerHTML = `<img class="real-poster-image" src="${m.poster}" alt="${escapeHtml(m.title)} poster" loading="eager"
          onerror="this.style.display='none'; this.parentElement.classList.add('poster-image-missing');">`;
        poster.classList.add('has-real-poster');
      } else {
        poster.classList.remove('has-real-poster');
        poster.innerHTML = `<div class="ph-mark"><div class="ph-glyph">${escapeHtml(m.glyph || m.title[0].toUpperCase())}</div><div class="ph-title">${escapeHtml(m.title)}</div><div class="ph-industry">${escapeHtml(m.language)} · ${m.year}</div></div>`;
      }
    }
    ...
    setHtml('alsoLikeList', related.map(r => `
      <a href="movie.html?id=${r.id}" style="display:flex;gap:10px;align-items:center;">
        <div style="width:44px;height:64px;border-radius:4px;overflow:hidden;flex-shrink:0;" class="ph-poster related-poster" data-tone="${r.posterTone || 1}">
          ${r.poster ? `<img class="real-poster-image" src="${r.poster}" alt="${escapeHtml(r.title)} poster" loading="lazy" onerror="this.style.display='none';">` : ''}
        </div>
  ...
  function render(results, query) {
    ...
    suggestionsEl.innerHTML = results.map(m => `
      <a class="sg-item" href="movie.html?id=${m.id}">
        <div class="sg-thumb ph-poster" data-tone="${m.posterTone || 1}">
          ${m.poster ? `<img class="real-poster-image" src="${m.poster}" alt="${escapeHtml(m.title)} poster" loading="lazy" onerror="this.style.display='none';">` : ''}
        </div>
        <div>
